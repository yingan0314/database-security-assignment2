...
if (isset($_POST['add'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO menu (food_name, price, description, image, category) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sdsss", $name, $price, $desc, $path, $category);
    $stmt->execute();
...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE menu SET food_name=?, price=?, description=?, image=?, category=? WHERE food_id=?");
    $stmt->bind_param("sdsssi", $name, $price, $desc, $path, $category, $id);
    $stmt->execute();
...
/* =========================
   FILTER CATEGORY - MySQL Version
========================= */
$category = $_GET['category'] ?? 'all';

if ($category == 'all') {
    $stmt = $conn->prepare("SELECT * FROM menu ORDER BY food_id DESC");
} else {
    $stmt = $conn->prepare("SELECT * FROM menu WHERE category=? ORDER BY food_id DESC");
